...
<script>
    // -------------------------
    // Calendar JS (safe, integrated)
    // - Renders into #calendarDays (keeps your UI)
    // - Uses English month names
    // - Keeps your emoji rule:
    //     Today => 🔥 (today class)
    //     Past streak => 👏 (completed + background)
    //     Past non-streak => number only
    //     Future => faded (future class)
    // - Safe: checks element exists before touching it
    // -------------------------
    ...
    (function () {
        ...
        // Sample streak data - Replace with backend data as needed
        const streakDays = [
        ...
        function safeGet(id) {
            ...
            if (!el) console.warn(`#${id} not found on the page.`);
            return el;
        }

        function renderCalendar() {
            const calendarDays = safeGet('calendarDays');
            const monthDisplay = safeGet('currentMonth');
            if (!calendarDays || !monthDisplay) return;

            // Update month display
            monthDisplay.textContent = `${monthNames[currentMonth]} ${currentYear}`;

            // Clear calendarDays safely
            // (we only clear the inner container - other elements on page unaffected)
            calendarDays.innerHTML = '';

            // Calculate first day (0 = Sunday) and number of days in month
            const firstDay = new Date(currentYear, currentMonth, 1).getDay();
            const daysInMonth = new Date(currentYear, currentMonth + 1, 0).getDate();

            // Add leading empty cells (so first date falls under correct weekday)
            for (let i = 0; i < firstDay; i++) {
                const emptyDay = document.createElement('div');
                emptyDay.className = 'calendar-day empty';
                // ensure visible grid sizing even if CSS uses visibility:hidden
        ...
        // Initialize on DOMContentLoaded (safe)
